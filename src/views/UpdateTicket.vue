      ...
      <div class="row justify-content-center align-items-center">
        <div class="col-16 col-md-12 col-lg-12" id="create_ticket_div">
          ...
          <div class="button-close-div">
            <button id="close-btn" class="btn btn-danger" v-on:click="backToHome">Close</button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    updateTicket (e) {
      ...
      if (!this.isFormForNewFlightHidden) {
        ...
        if (this.from === this.to) {
          alert("Origin city and destination city can't be the same!");
          return;
        }
      }
      ...
      if (!this.isReturnDateHidden) {
        ...
        if (new Date(this.departDate).getTime() >= new Date(this.returnDate).getTime()) {
          alert("Depart date can't be after return date!");
          return;
        }
      }

      if (this.companyId === "" || this.count === "") {
        alert("You must fill in all fields!");
        return;
      }
